...
const queryValidations = [
	...
	query('role').optional({ checkFalsy: true }).not().isArray().withMessage(i18n.__('must be an string')),
...
const bodyValidationsPost = [
	...
	body('role').not().isArray().not().isEmpty().withMessage(i18n.__('is required'))
];
...
const bodyValidationsPut = [
	...
	body('role').optional().not().isArray().not().isEmpty().withMessage(i18n.__('is required'))
];
